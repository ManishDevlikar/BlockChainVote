...
class VotingList extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      console.log(add);
      // 0x8507DcC3d362d7d0fA11eF24cbF0B739FE9beea9
      ...
      // console.log(await election.methods.getCandidate(0).call());
      // const c = await election.methods.getNumOfCandidates();
      const c = await election.methods.getNumOfCandidates().call();
      // console.log(await election.methods);
      console.log(c);
      

      const candidateList = await election.methods.getCandidate(1).call();
      console.log(candidateList);
      let candidates = [];
      for (let i = 0; i < c; i++) {
        candidates.push(await election.methods.getCandidate(i).call());
        console.log(await election.methods.getCandidate(i).call());
      }
